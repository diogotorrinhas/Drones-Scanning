...
def on_message(client, userdata, message):
    ...
    if topic == "vanetza/out/cam":
        ...
        print("SCRIPT FOR OBU1 -> LAT: "+str(lat)+"| LON: "+str(lon)+ " FROM OBU ("+str(ip)+", "+str(brk)+") | RECEIVER: "+str(ip_received))
...
def update_zones_db_with_actualCoordinatesOfOBU1(lat, lon, ip, brk):
    for zone in analyze_zones:
        proximity = check_proximity(lat, lon, zone, 15) #15 meters of proximity
        #print("PROXIMITY FOR OBU "+str(client._host)+" : "+str(proximity))
        if proximity == True:
