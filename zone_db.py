...
def create_zones_table():
    db = sql.connect('zones.db')

    db.execute('''drop table if exists zones''')
    db.execute('''create table zones(
            ip text,
            broker text,
            water text,
            latitude_zone real,
    ...
    db.execute('''insert into zones values(null, null, null, 40.63239230194507, -8.6603676549793, 0)''')
    db.execute('''insert into zones values(null, null, null, 40.63195486065902, -8.661093930263293, 1)''')
    db.execute('''insert into zones values(null, null, null, 40.63142859277966, -8.660753978842079, 2)''')
    db.execute('''insert into zones values(null, null, null, 40.63192425397194, -8.659820484340262, 3)''')
    db.execute('''insert into zones values(null, null, null, 40.63284006630615, -8.660225733927449, 4)''')
...
def update_zones_db_watering(zone, causecode):
    ...
    conn = sql.connect('zones.db')

    # Create a cursor to execute SQL queries
    cursor = conn.cursor()
    
    # Execute the UPDATE statement
    update_query = "UPDATE zones SET water = ? WHERE latitude_zone = ? AND longitude_zone = ?"
...
def update_zones_db_watering_by_ip(ip, causecode, latitude, longitude):
    ...
    update_query = "UPDATE zones SET water = ? WHERE ip = ? AND ABS(latitude_zone - ?) <= ? AND ABS(longitude_zone - ?) <= ?"
...
def update_zones_db(ip, broker, latitude, longitude):
    ...
    select_query = "SELECT ip, broker FROM zones WHERE latitude_zone = ? AND longitude_zone = ?"
    ...
    # Execute the UPDATE statement
    update_query = "UPDATE zones SET ip = ?, broker = ? WHERE latitude_zone = ? AND longitude_zone = ?"
...
def check_zones_not_occupied():
    ...
    select_query = "SELECT * FROM zones WHERE ip IS NULL and broker IS NULL"
...
def get_ID_by_coordinates(zone):
    ...
    # Select the zones with non-null IP and broker
    select_query = "SELECT id FROM zones WHERE latitude_zone = ? AND longitude_zone = ?"
...
def return_zone_associated_to_OBU(zone):
    ...
    select_query = "SELECT * FROM zones WHERE latitude_zone = ? AND longitude_zone = ?"
    cursor.execute(select_query, (zone[0],zone[1]))
